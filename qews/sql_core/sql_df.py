...
class PandaSQL:
    ...
    def __init__(self, sqlite3_connection,  persist=False):
        """
        Initialize with a specific database.
        """

        self.sqlite3_connection = sqlite3_connection

        self.engine = create_engine('sqlite://', poolclass=NullPool,
                  creator=self.sqlite_engine_creator)
    ...
    def sqlite_engine_creator(self):
        # self.sqlite3_connection = sqlite3.connect(":memory:")
        return self.sqlite3_connection
...
def sqldf(sqlite3_connection, query,  env=None):
    return PandaSQL(sqlite3_connection)(query, env)
